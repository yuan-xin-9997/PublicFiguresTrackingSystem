...
同一收件人被多条重叠规则命中时，对规则结果取并集，因此一个任务运行中的同一事件只出现一次。选择关联表而不是在 `timeline_events` 增加单一 `task_run_id`，是因为未来一个事件可能从多个运行获得证据，而本功能只需要保存“首次由本运行新建”的事实。
...
只发送本次运行新建且类型命中规则的事件。规则在任务结束创建 outbox 时求值并固化事件列表，之后修改规则不会改变已排队批次。邮件内容在实际发送时从事件表读取最新可见字段；已被驳回或删除的事件在发送前跳过并记录原因，空批次标记为 `skipped`。
...
- `GET /api/v1/notifications/options`（可选采集任务、事件类型）
...
Vue 增加“推送管理”导航和四个区域：生效邮件配置、规则编辑、测试发送、投递记录。任务中心的任务运行日志继续显示本次排队数量和最近失败摘要，并提供跳转。
