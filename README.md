...
- 原始文档去重、任务运行记录及逐条日志。
- 本地确定性抽取与可选 OpenAI-compatible 外部模型；按行动/发言主语归属人物，并从事件相邻句补充公开地点。
...
| 时间线 | 按人物、类型、确认状态和关键词筛选三类事件，卡片显示具体来源名称 |
...
5. 使用 `rsync` 更新代码，保留 `data/`、`logs/` 和服务器已有的 `config/app.json`；首次部署才复制默认配置。
6. 启动服务并执行就绪检查。
