...
- 搜索、地点兼容视图、仪表盘和审计日志。
- 邮件事件推送：规则可选任务结束后即时发送，或在一个/多个北京时间汇总自上次入库截止点以来的新事件；支持持久化重试与投递记录。
...
| 推送管理 | 配置邮件通道、即时/定时增量规则；预览入库窗口、立即汇总、查看或重试批次；维护按发生时间汇总的每日时间线邮件 |
...
| `notifications.scheduled_incremental` | 定时增量时区（默认 `Asia/Shanghai`）、默认发送时刻列表（`["08:30"]`）和调度轮询 |
...
事件推送规则的“投递模式”可改为“定时增量汇总”，并配置一个或多个北京时间 `HH:mm`。系统用 `task_run_events` 的入库三元组（入库时间、任务运行 ID、事件 ID）记录截止点；每次只选择上次截止点之后、当前固定上界之前首次新建且匹配任务、人物和类型的事件。事件的实际发生日期不决定窗口，因此今天才抓到的历史日期事件仍会进入下一封增量邮件。

新建、从即时切换、重新启用或修改任务/人物/类型范围时，起点重置为操作当时的当前高水位，默认不补发存量或停用期间事件；只改名称或发送时刻会保留待推增量。页面会在可能重置游标时确认，可先“预览待推窗口”，也可“立即汇总”并推进截止点。

运行、候选事件、收件人和分片在一个 SQLite 短事务内固化，游标随运行快照成功创建而推进。SMTP 失败只重试原批次，不回退游标或重新选择事件；后续窗口不会与失败窗口重叠。没有候选时只记录 `empty`。应用长时间停止后只建立最近一个到期运行，但该运行会覆盖整个尚未切分的入库范围。
...
候选事件必须匹配规则人物和类型，且未被删除或历史驳回。窗口归属优先使用事件发生时间；发生时间未知时使用入库时间确定所属日报，但邮件仍显示“时间未知”。正文按已知事件时间升序排列，未知时间置后，并以人物、类型和事件 ID 保持稳定顺序。
...
- 定时增量调度：应用进程内 `ScheduledIncrementalScheduler`，与采集调度独立；运行详情保留上下界、候选/批次及发送/失败/跳过计数。
...
SQLite 备份中的页面 SMTP 密码是密文。`PFTS_NOTIFICATION_CREDENTIAL_KEY` 必须独立安全保存；密钥丢失时只能清除页面密码并重新录入。轮换主密钥前先记录 SMTP 密码，在页面清除旧密文、替换环境密钥后再保存。备份会同时保留即时、定时增量和日报规则、运行、批次与明细；恢复后未完成批次由 Worker 继续处理。邮件失败不会回滚采集结果，可在“推送管理”查看清洗后的错误摘要。

Linux 生产环境可从 `src/JenkinsConfig/pfts.service.example` 生成 systemd unit：把 `__DEPLOY_DIR__` 和 `__RUN_USER__` 替换为实际值后安装到 `/etc/systemd/system/pfts.service`，执行 `systemctl daemon-reload && systemctl enable --now pfts`。unit 使用部署目录下的 `.venv` 和可选 `data/runtime.env`，停止服务时 FastAPI lifespan 会依次停止采集、日报、定时增量调度器和邮件 Worker。
...
7. 启动服务并执行就绪检查、即时邮件、日报和定时增量只读冒烟检查。
