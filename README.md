...
- 文档发布时间优先取文章 URL 中的发布日期路径（`/20260803/`、`/2026/0803/`、`/2026/08/03/`），其次结构化提取结果，最后才回退到正文完整年月日，并跳过「施行/生效/将于」等未来语境日期，避免把条例施行日当成发布日；事件发生时间一律取该发布时间（`published_at`，缺失时回退 `collected_at`），不再从事件正文中抽取或推断日期，时间精度有发布时间时记 `day`，无法解析时记 `unknown`。
- 原始文档去重、任务运行记录及逐条日志。
