...
| 层次 | 重点 |
|---|---|
| 单元 | 配置覆盖/脱敏、邮件凭证加密、SMTP 模式/重试、密码哈希、时间转换、URL 安全、RSS 解析、主体归属反例、中国日报正文清洗、去重键 |
| 集成 | Schema 初始化、用户同步、会话、CRUD、任务运行、task-run 新事件、邮件 outbox/权限、文档到事件证据链、外部模型回退 |
| API | 401/403、分页筛选、输入校验、人工审核、配置脱敏、维护 dry-run/事务回滚/锁定保护/幂等 |
| 前端 | API 错误、北京时间格式、状态标签、筛选参数、管理员维护入口与确认 |
