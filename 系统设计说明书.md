...
`analyze_document` 仅在真正插入新 `timeline_events` 时，把事件写入 `task_run_events`；命中 `dedup_key` 或相似事件只追加证据，不建立新建关联。任务状态提交后，服务读取已启用规则，按任务和事件类型筛选本运行的新事件，对收件人取并集并按稳定事件 ID 分片写入 SQLite outbox。排队异常单独写任务日志，不回滚采集结果。
