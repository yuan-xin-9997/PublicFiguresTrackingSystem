...
- 审计日志覆盖认证、用户权限、人物、来源、任务运行、事件审核和两类数据质量维护执行。

## 13. 备份与恢复

备份脚本使用 Python SQLite Backup API 生成一致性数据库副本，同时复制配置、密码定义和下载目录。恢复前停止服务，校验目标目录，并保留当前数据的回滚副本。执行存量数据质量维护前必须先备份，再 dry-run 保存人物/来源范围、计数和样例；执行事务先删除无效证据，只有未人工锁定且完全失去证据的事件才删除。中国日报合法文章清洗后保留原始材料记录并重新分析，聚合页才删除。异常时停止服务、回滚代码并恢复执行前数据库副本。MVP 文档提供手工步骤；自动轮转备份作为后续增强。
...
| 层次 | 重点 |
|---|---|
| 单元 | 配置覆盖/脱敏、密码哈希、时间转换、URL 安全、RSS 解析、主体归属反例、中国日报正文清洗、去重键 |
| 集成 | Schema 初始化、用户同步、会话、CRUD、任务运行、文档到事件证据链、外部模型回退 |
