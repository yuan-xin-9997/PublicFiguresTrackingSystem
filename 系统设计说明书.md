...
发布日期按“结构化提取结果、正文中的完整日期时间、文章 URL 日期路径”依次回退。事件段落自身没有日期时继承原始文档发布日期，并保留日级时间精度。
...
本地抽取器使用人物姓名/别名确认相关性，以可配置关键词区分行程和言论，识别常见 ISO/中文日期及直接引语；其余明确相关事实归入“其他”。无法确定的时间和坐标保持为空。
